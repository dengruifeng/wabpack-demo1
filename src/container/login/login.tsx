...
import { Form, Button, Input, Row, InputNumber, message } from 'antd';
import { FormMap } from './config';
import JSEncrypt from 'jsencrypt';
import { getPublicKey, userLogin } from 'api/login';
...
export const LoginForm: React.FC<any> = () => {
  ...
  const handleSubmit = async (e: { userName: string; password: string }) => {
    ...
    // console.log('加密', rsaPassWord, 'base64加密', Base64.encode(rsaPassWord), 'atob', Base64.atob('123'));
    // // 解密
    ...
    // const uncrypted = encryptor.decrypt(rsaPassWord);//解密之前拿公钥加密的内容
